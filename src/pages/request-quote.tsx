...
const RequestQuote: React.FC = () => {
  ...
  const [pendingProduct, setPendingProduct] = useState<{product: string, color: string} | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!prod) return;
    const color = prod.colorOptions.length > 0 ? (pendingProduct.color || prod.colorOptions[0]) : '';
    const alreadyExists = selectedProducts.some(
      (sp) => sp.name === prod.name && normalizeColor(sp.color) === normalizeColor(color)
    );
    if (!alreadyExists) {
  ...
  useEffect(() => {
    ...
    if (prod && prod.colorOptions && prod.colorOptions.length > 0) {
      const availableColors = prod.colorOptions.filter((c) => !selectedProducts.some((sp) => sp.name === prod.name && sp.color === c));
  ...
  function getAvailableProducts(selectedProducts: QuoteProduct[]) {
    return productsData.filter((p) => {
      if (Array.isArray(p.colorOptions) && p.colorOptions.length > 0) {
        const availableColors = p.colorOptions.filter((color) => !selectedProducts.some((sp) => sp.name === p.name && sp.color === color));
        return availableColors.length > 0;
      } else {
        return !selectedProducts.some((sp) => sp.name === p.name && normalizeColor(sp.color) === '');
      }
    });
  }
  ...
  const handleAddProduct = () => {
    ...
    const selectedKeys = new Set(selectedProducts.map(sp => `${sp.name}|||${normalizeColor(sp.color)}`));
  ...
  return (
    <div className="max-w-2xl mx-auto py-16 px-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
            ...
            <div className="flex-1">
              ...
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={noMoreProducts ? "No more available products" : "Add a product"} />
                </SelectTrigger>
                <SelectContent>
                  {availableProducts.map((product) => {
                    let label = product.name;
                    let availableColors = Array.isArray(product.colorOptions)
                      ? product.colorOptions.filter((color) => !selectedProducts.some((sp) => sp.name === product.name && sp.color === color))
                      : [];
              ...
              {noMoreProducts && (
                <div className="text-sm text-gray-500 mt-2">All products have been added to the quote request.</div>
              )}
            </div>
            {productToAdd && (() => {
              let name = productToAdd;
              ...
              if (prod && prod.colorOptions && prod.colorOptions.length > 0) {
                  ...
                  <>
                    ...
                    {!onlyOneColor && (
                      <div>
                        ...
                        <Select value={colorToAdd} onValueChange={setColorToAdd}>
                    ...
                    <Button type="button" onClick={handleAddProduct} className="ml-2" disabled={!colorToAdd}>Add</Button>
                  </>
                );
              } else {
                ...
                return (
                  <>
                    ...
                    <Button type="button" onClick={handleAddProduct} className="ml-2">Add</Button>
                  </>
                );
              }
            })()}
          ...
          <div className="space-y-4">
            {selectedProducts.map((sp, idx) => {
              ...
              return (
                <Card key={sp.name + '|||' + colorStr} className="flex items-center gap-4 p-4">
                  <img src={prod?.images?.[0] || ''} alt={sp.name} className="w-12 h-12 object-cover rounded" />
                  <div className="flex-1">
                    <div className="font-semibold">{sp.name}{colorStr ? ` (${colorStr})` : ''}</div>
                    <div className="flex gap-4 mt-1">
                      ...
                      {prod && Array.isArray(prod.colorOptions) && prod.colorOptions.length > 0 && (() => {
                        const availableColors = prod.colorOptions.filter((color) => !selectedProducts.some((spp, i) => i !== idx && spp.name === prod.name && normalizeColor(spp.color) === color) || color === colorStr);
                        if (availableColors.length === 1) {
                          // If only one color, do not show picker, but auto-set color if not already set
                          if (colorStr !== availableColors[0]) {
                            setTimeout(() => handleProductChange(idx, 'color', availableColors[0]), 0);
                          }
                          return null;
                        }
                        return (
                          <div>
                            ...
                            <Select
                              ...
                              onValueChange={(val) => handleProductChange(idx, 'color', val)}
                            >
                              <SelectTrigger className="w-28">
                                <SelectValue placeholder="Select color" />
                              </SelectTrigger>
        ...
        <Button type="submit" className="w-full py-3 font-semibold text-lg mt-8">Submit Quote Request</Button>
      </form>
    </div>
  );
};
